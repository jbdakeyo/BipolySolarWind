...
def solve_bipoly(N, L, gamma_p_max, gamma_e_max, Tpc, Tec, r_poly_p, r_poly_e
    ...
    if(plot_unT):
        ...
        plt.scatter(rc_iso, cs_T[ind_rc_poly], s=50, marker='o', color='black', label='Sonic point', zorder=2)
        ...
        ###############################################
        plt.subplot(1,3,3)
        plt.plot(r[r<r_poly_p], Tp[r<r_poly_p], color = color_reg[0], linewidth = ep_trait )
        plt.plot(r[r>=r_poly_p], Tp[r>=r_poly_p], color = color_reg[-1], linewidth = ep_trait )
        plt.plot(r[r<r_poly_e], Te[r<r_poly_e], '--', color = color_reg[0], linewidth = ep_trait )
        plt.plot(r[r>=r_poly_e], Te[r>=r_poly_e], '--', color = color_reg[-1], linewidth = ep_trait )
        plt.xlabel('Radius ($r \: / \: r_\\odot$)', fontsize=pol)
